...
export function MobileNavBar({ user }: MobileNavBarProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-black/95 backdrop-blur-xl border-t border-primary/20 safe-area-bottom">
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive ? "text-neon-glow bg-primary/12" : "text-muted-foreground"
              )}
        ...
        <Sheet>
          <SheetTrigger asChild>
            <button
              className={cn(
                "flex flex-col items-center justify-center flex-1 h-full gap-1 transition-colors rounded-xl",
                isMoreActive ? "text-neon-glow bg-primary/12" : "text-muted-foreground"
              )}
          ...
          <SheetContent side="bottom" className="h-auto max-h-[85vh] rounded-t-3xl bg-[#09070f] border-primary/30">
            <SheetHeader className="pb-4">
              ...
              <div className="flex items-center gap-3 pt-2">
                <Avatar className="h-12 w-12 border border-primary/30">
                  ...
                  />
                    <AvatarFallback className="bg-primary/15 text-neon-glow">{initials}</AvatarFallback>
                </Avatar>
              ...
              {moreMenuItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    className={cn(
                      "flex flex-col items-center justify-center gap-2 p-3 rounded-xl transition-colors relative border",
                      isActive
                        ? "bg-primary/14 text-neon-glow border-primary/35"
                        : "text-muted-foreground hover:bg-primary/10 border-transparent hover:border-primary/20"
                    )}
            ...
            <div className="border-t border-primary/20 pt-4 pb-2">
              <button
                onClick={logout}
                className="flex items-center gap-3 p-3 rounded-xl text-muted-foreground hover:bg-red-500/10 hover:text-red-300 transition-colors w-full text-left border border-transparent hover:border-red-500/30"
