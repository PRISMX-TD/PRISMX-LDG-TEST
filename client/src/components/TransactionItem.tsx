...
function TransactionItemInner({
  ...
  return (
    <div
      className={`flex items-center gap-3 py-3 px-4 rounded-2xl border transition-all cursor-pointer ${getBgColor()} ${
        isExpense ? "border-expense/20 hover:border-expense/35" : isIncome ? "border-income/20 hover:border-income/35" : "border-transfer/25 hover:border-transfer/40"
      }`}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="font-medium text-sm text-foreground truncate" data-testid={`text-description-${transaction.id}`}>
          {getDescription()}
        </p>
