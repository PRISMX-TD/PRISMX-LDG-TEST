    ...
    <div className="glass-card mb-6">
      <div className="p-6 border-b border-primary/15 flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button className="p-1.5 hover:bg-primary/10 rounded-md text-muted-foreground hover:text-white transition-colors border border-transparent hover:border-primary/20">
      ...
      <div className="w-full overflow-x-auto">
        <table className="w-full text-left">
          <thead className="bg-primary/7 text-muted-foreground text-xs uppercase tracking-wider">
            <tr>
          ...
          <tbody className="text-sm divide-y divide-primary/10">
            {visible.map((t) => {
              ...
              return (
                <tr 
                  key={t.id} 
                  className="hover:bg-primary/[0.08] active:bg-primary/[0.12] transition-colors group cursor-pointer"
                  onClick={() => {
                ...
                >
                  <td className="px-6 py-4 flex items-center">
                    <div className="w-9 h-9 rounded-lg bg-black border border-primary/25 flex items-center justify-center mr-3 text-white">
                      <Icon className="w-4 h-4" style={{ color: t.category?.color }} />
                    </div>
                    <span className="text-foreground font-medium">{t.description || t.category?.name || "未命名交易"}</span>
                  </td>
                  ...
                  <td className="px-6 py-4 text-muted-foreground">{t.wallet?.name}</td>
      ...
      {transactions.length > VISIBLE_COUNT && (
        <div className="px-6 py-4 border-t border-primary/15 flex justify-end">
