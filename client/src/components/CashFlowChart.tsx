  ...
  if (!Recharts || !isVisible) {
    return (
      <div ref={containerRef} className="flex-[2] glass-card p-6 min-h-[300px] flex items-center justify-center">
        <div className="animate-pulse text-muted-foreground">加载图表组件...</div>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 sm:gap-0">
        <div>
          <h3 className="text-white font-semibold flex items-center gap-2">
            <ArrowUpRight className="w-4 h-4 text-neon-glow" />
            资金流向分析
          </h3>
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <button 
            ...
            className={cn(
              ...
              timeRange === "week" 
                ? "bg-primary/14 text-white border border-primary/30" 
      ...
      <div className="flex-1 min-h-[250px] w-full -ml-2" style={{ minHeight: '250px' }}>
        <ResponsiveContainer width="100%" height={250}>
          <AreaChart data={data}>
            ...
            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(139,92,246,0.14)" />
            <XAxis 
              dataKey="name" 
              axisLine={false} 
              tickLine={false} 
              tick={{fill: '#9aa0b3', fontSize: 12}} 
              dy={10}
            ...
            />
            <Tooltip labelStyle={{ color: "rgba(255,255,255,0.55)", fontSize: "11px", marginBottom: "4px" }} itemStyle={{ color: "rgba(255,255,255,0.92)" }} 
              contentStyle={{backgroundColor: '#0d0a16', border: '1px solid rgba(167,139,250,0.34)', borderRadius: '12px'}}
              itemStyle={{color: '#fff'}}
            />
            <Area 
              type="monotone" 
