...
export default function Transactions() {
  ...
  const PAGE_SIZE = 50;
  const {
    data: txPages, isLoading: isTxLoading, fetchNextPage, hasNextPage, isFetchingNextPage,
  } = useInfiniteQuery<TxRel[]>({
    ...
    getNextPageParam: (last, all) => last.length === PAGE_SIZE ? all.reduce((s, p) => s + p.length, 0) : undefined,
  ...
  const { data: stats } = useQuery<TxStats>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/transactions/stats?startDate=${statsRange.start.toISOString()}&endDate=${statsRange.end.toISOString()}`, { credentials: "include" });
          ...
          <div className="space-y-5">
            ...
            {hasNextPage && (
              <div className="flex justify-center py-4">
                <PillButton variant="ghost" onClick={() => fetchNextPage()} disabled={isFetchingNextPage}>
