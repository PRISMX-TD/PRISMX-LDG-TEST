...
import {
  ArrowLeft, Pencil, ChevronRight, Plus, Settings, ArrowUp, ArrowDown,
...
export default function WalletDetail() {
  ...
  const { data: allTransactions = [], isLoading: txLoading } = useQuery<Transaction[]>({
    ...
    queryFn: async () => {
      if (!walletId) return [];
      const r = await fetch(`/api/transactions?walletId=${walletId}&limit=500`, { credentials: "include" });
  ...
  return (
    <div className="text-foreground">
      <div className="max-w-4xl mx-auto px-4 md:px-6 py-5 md:py-7 pb-28 md:pb-10 space-y-5">
          ...
          <div className="relative">
            ...
            {walletLoading ? (
              ...
              <p className="leading-[1.05] tracking-tight m-0 flex items-baseline">
                <span className={`text-[44px] md:text-[52px] font-semibold ${balance < 0 ? "text-expense" : "num-gradient"}`}>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <RoundIconButton size="lg" onClick={() => setShowTx(true)} aria-label="收入">
                <ArrowDown className="w-5 h-5" />
              </RoundIconButton>
              <RoundIconButton size="lg" onClick={() => setShowTx(true)} aria-label="支出">
