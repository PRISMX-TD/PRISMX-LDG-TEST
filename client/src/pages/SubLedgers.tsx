...
export default function SubLedgers() {
  ...
  const getConvertedAmount = (t: Transaction): number => {
    ...
    // 使用钱包设置的汇率进行转换 (Wallet Currency -> Default Currency)
  ...
  const getSubLedgerStats = (subLedger: SubLedger) => {
    const subLedgerTransactions = transactions.filter(t => t.subLedgerId === subLedger.id);
    let income = 0;
    let expense = 0;
    ...
    subLedgerTransactions.forEach(t => {
      ...
      // 原始币种统计 (仅当交易币种 == 子账本币种时累加)
      // 如果交易币种不一致，我们这里简单处理：不计入或者尝试转换
      // 更好的做法：如果子账本设了 JPY，我们只累加 JPY 的交易
      // 或者：如果交易记录了 originalAmount 且 currency == JPY，用 originalAmount
      // 否则如果 wallet.currency == JPY，用 amount
      
      const targetCurrency = subLedger.currency || "MYR";
      let targetAmount = 0;
      
      if (t.currency === targetCurrency) {
        // 交易原币种就是目标币种 (cross-currency transaction)
        if (t.originalAmount) {
          targetAmount = parseFloat(t.originalAmount);
        } else {
          // 非跨币种，直接用 amount (此时 wallet currency 应该也是 targetCurrency)
           targetAmount = parseFloat(t.amount);
        }
      } else {
        const wallet = wallets.find(w => w.id === t.walletId);
        if (wallet && wallet.currency === targetCurrency) {
           targetAmount = parseFloat(t.amount);
        }
      }
    ...
    return { 
      count: subLedgerTransactions.length, 
      income, 
      expense, 
      balance: income - expense,
      originalIncome,
      originalExpense,
      originalBalance: originalIncome - originalExpense
    };
  };
            ...
            {activeSubLedgers.length === 0 && !showArchived ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {activeSubLedgers.map((subLedger) => {
                  const stats = getSubLedgerStats(subLedger);
                  return (
                    <Card
                      key={subLedger.id}
                      className="glass-card border-primary/20 hover-elevate relative overflow-visible h-full min-h-[340px] flex flex-col cursor-pointer transition-all hover:border-primary/40"
                      data-testid={`card-subledger-${subLedger.id}`}
                      onClick={() => setLocation(`/sub-ledgers/${subLedger.id}`)}
                    >
                      <div
                          ...
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button 
                                ...
                                size="icon" 
                                className="shrink-0" 
                                data-testid={`button-menu-subledger-${subLedger.id}`}
                      ...
                      <CardContent className="space-y-5 px-5 pb-5 flex-1 flex flex-col">
                        ...
                        <div className="flex items-center justify-between text-sm py-1">
                          ...
                          <div className="flex items-center gap-1.5 text-muted-foreground bg-white/5 px-2.5 py-1 rounded-md">
                            <span className="text-xs font-medium">{subLedger.currency || "MYR"}</span>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-white/5 border border-white/5">
                          <div>
                            ...
                            {/* 显示原始币种辅助信息 */}
                            {subLedger.currency && subLedger.currency !== (user?.defaultCurrency || "MYR") && stats.originalIncome > 0 && (
                               <p className="text-[10px] text-muted-foreground/70 mt-0.5">
                                 ({stats.originalIncome.toFixed(2)} {subLedger.currency})
                               </p>
                            )}
                          </div>
                          <div className="text-right">
                            ...
                            {/* 显示原始币种辅助信息 */}
                            {subLedger.currency && subLedger.currency !== (user?.defaultCurrency || "MYR") && stats.originalExpense > 0 && (
                               <p className="text-[10px] text-muted-foreground/70 mt-0.5">
                                 ({stats.originalExpense.toFixed(2)} {subLedger.currency})
                               </p>
                            )}
                          </div>
                        </div>
