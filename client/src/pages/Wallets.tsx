...
import {
  ...
  Banknote, CreditCard, Smartphone, TrendingUp, ChevronRight,
...
export default function Wallets() {
  ...
  function deleteWithUndo(w: WalletType) {
    void undoableDelete({
      deleteUrl: `/api/wallets/${w.id}`,
      restoreUrl: "/api/wallets",
      restorePayload: {
        name: w.name, type: w.type, currency: w.currency, color: w.color, icon: w.icon,
        exchangeRateToDefault: w.exchangeRateToDefault, isFlexible: w.isFlexible,
      },
      invalidateKeys: [["/api/wallets"]],
      label: `已删除 "${w.name}"`,
    });
  }

  const visible = wallets.filter(w => !(w.isFlexible === false && (w.name || '').endsWith(' (归档)')));
  ...
  return (
    <div className="text-foreground">
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-5 md:py-7 pb-28 md:pb-10 space-y-5">
        ...
        <section className="hero-card">
          <div className="relative">
            ...
            <p className="leading-[1.05] tracking-tight m-0 flex items-baseline">
              <span className="text-[44px] md:text-[52px] font-semibold num-gradient">
            ...
            <div className="flex items-center gap-2.5 mt-5">
              ...
              <RoundIconButton size="lg" onClick={() => setIsTxOpen(true)} aria-label="记一笔">
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                                  ...
                                  <DropdownMenuItem
                                    ...
                                    onSelect={(e) => { e.preventDefault(); if (confirm(`删除 "${w.name}"？关联交易保留但失去关联（30 秒可撤销）`)) deleteWithUndo(w); }}
                                  >
