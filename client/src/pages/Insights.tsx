...
import { BarChart3, FileText, BookOpen, Loader2, ArrowLeft, Compass } from "lucide-react";

/* r7 — Insights hub rewritten from scratch.
   Page shell with own gradient background and pill tab nav. No PageContainer. */

const Analytics  = lazy(() => import("./Analytics"));
const Reports    = lazy(() => import("./Reports"));
const SubLedgers = lazy(() => import("./SubLedgers"));

type Tab = "analytics" | "reports" | "subledgers";
const TABS: { id: Tab; label: string; icon: any; accent: string }[] = [
  { id: "analytics",  label: "数据分析", icon: BarChart3, accent: "#a78bfa" },
  { id: "reports",    label: "财务报表", icon: FileText,  accent: "#fbbf24" },
  { id: "subledgers", label: "子账本",   icon: BookOpen,  accent: "#34d399" },
];

export default function Insights() {
  const [, setLocation] = useLocation();
  const sp = new URLSearchParams(useSearch());
  const initial = (sp.get("tab") as Tab) || "analytics";
  ...
  const setActive = (t: Tab) => {
    ...
    next.set("tab", t);
    setLocation(`/insights?${next.toString()}`, { replace: true });
  };
  ...
  return (
    <div className="min-h-screen text-foreground relative">
      <div aria-hidden className="fixed inset-0 -z-10 pointer-events-none">
        ...
        <div className="absolute bottom-0 right-0 w-[460px] h-[460px] rounded-full opacity-25 blur-3xl"
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-5 md:py-8 pb-28 md:pb-12 relative">
        <header className="mb-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="text-[11px] tracking-[0.2em] uppercase text-foreground/45 m-0">Insights</p>
              <h1 className="text-[24px] md:text-[32px] font-bold tracking-tight m-0 flex items-center gap-2">
                <Compass className="w-6 h-6 text-[#a78bfa]" />洞察
              </h1>
              <p className="text-[12.5px] text-foreground/55 m-0 mt-0.5">看清楚钱的流向 · 月度对比 · 按项目独立追踪</p>
            </div>
          </div>
        ...
        <Suspense fallback={<div className="py-20 flex items-center justify-center"><Loader2 className="w-6 h-6 animate-spin text-[#a78bfa]" /></div>}>
          {tab === "analytics"  && <Analytics />}
          {tab === "reports"    && <Reports />}
          {tab === "subledgers" && <SubLedgers />}
        </Suspense>
      </div>
    </div>
  );
