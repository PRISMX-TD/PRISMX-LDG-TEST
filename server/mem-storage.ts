...
import { type IStorage, type TransactionFilters, type TransactionStats, type BudgetWithSpending, type WalletDelta } from "./storage";
...
export class MemStorage implements IStorage {
  ...
  async getUserByEmail(email: string) {
    return Array.from(this.users.values()).find(u => (u as any).email === email);
  }
  async upsertUser(user: any) {
    const id = user.id || `mem-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const existing = this.users.get(id);
    const u = { ...(existing || {}), ...user, id, createdAt: existing?.createdAt || new Date(), updatedAt: new Date(), defaultCurrency: user.defaultCurrency || existing?.defaultCurrency || "MYR" };
    this.users.set(id, u);
    await this.initializeUserDefaults(id, u.defaultCurrency);
    return u;
  }
  ...
  async updateUserPassword(userId: string, passwordHash: string) {
    const u = this.users.get(userId);
    if (u) { (u as any).passwordHash = passwordHash; (u as any).updatedAt = new Date(); this.users.set(userId, u); }
  }

  // Password reset tokens (in-memory)
  private passwordResetTokens = new Map<string, any>();
  async createPasswordResetToken(token: any) { this.passwordResetTokens.set(token.tokenHash, { ...token, id: Date.now(), createdAt: new Date() }); }
  async getPasswordResetToken(tokenHash: string) { return this.passwordResetTokens.get(tokenHash); }
  async deletePasswordResetTokens(userId: string) {
    for (const [k, v] of Array.from(this.passwordResetTokens.entries())) {
      if (v.userId === userId) this.passwordResetTokens.delete(k);
    }
  }

  async getWallets(userId: string, includeArchived: boolean = false) {
    return Array.from(this.wallets.values())
      .filter(w => w.userId === userId && (includeArchived || !w.isArchived));
  }
  async getWallet(id: number, userId: string) {
  ...
  async incrementWalletBalance(id: number, userId: string, delta: number) {
    const w = this.wallets.get(id);
    if (w && w.userId === userId) {
      const next = (parseFloat(w.balance || "0") + delta).toFixed(2);
      const nw = { ...w, balance: next };
      this.wallets.set(id, nw);
      return nw;
    }
    return undefined;
  }
  ...
  async getCategory(id: number, userId: string) { const c = this.categories.get(id); return c && c.userId === userId ? c : undefined; }
  ...
  async getTransaction(id: number, userId: string) { const t = this.transactions.get(id); return t && t.userId === userId ? t : undefined; }
  ...
  async getTransactionsByTag(userId: string, tag: string) {
    return Array.from(this.transactions.values())
      .filter(t => t.userId === userId && Array.isArray((t as any).tags) && (t as any).tags.includes(tag))
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }
  // Single-threaded JS: sequential awaits with no interleaving == atomic enough for the mock.
  async createTransactionWithEffects(transaction: any, deltas: WalletDelta[]) {
    const created = await this.createTransaction(transaction);
    for (const d of deltas) if (d && d.delta) await this.incrementWalletBalance(d.walletId, transaction.userId, d.delta);
    return created;
  }
  async updateTransactionWithEffects(id: number, userId: string, data: any, reversalDeltas: WalletDelta[], applyDeltas: WalletDelta[]) {
    for (const d of reversalDeltas) if (d && d.delta) await this.incrementWalletBalance(d.walletId, userId, d.delta);
    const updated = await this.updateTransaction(id, userId, data);
    if (!updated) throw new Error("Transaction not found during atomic update");
    for (const d of applyDeltas) if (d && d.delta) await this.incrementWalletBalance(d.walletId, userId, d.delta);
    return updated;
  }
  async deleteTransactionWithEffects(id: number, userId: string, reversalDeltas: WalletDelta[]) {
    const ok = await this.deleteTransaction(id, userId);
    if (!ok) return false;
    for (const d of reversalDeltas) if (d && d.delta) await this.incrementWalletBalance(d.walletId, userId, d.delta);
    return true;
  }
  ...
  async getBudget(id: number, userId: string) { const b = this.budgets.get(id); return b && b.userId === userId ? b : undefined; }
  ...
  async getSavingsGoal(id: number, userId: string) { const s = this.savingsGoals.get(id); return s && s.userId === userId ? s : undefined; }
  ...
  async getRecurringTransaction(id: number, userId: string) { const r = this.recurringTransactions.get(id); return r && r.userId === userId ? r : undefined; }
  ...
  async getBillReminder(id: number, userId: string) { const b = this.billReminders.get(id); return b && b.userId === userId ? b : undefined; }
  ...
  async getExchangeCredential(id: number, userId: string) { const e = this.exchangeCredentials.get(id); return e && e.userId === userId ? e : undefined; }
  ...
  async getSubLedger(id: number, userId: string) { const s = this.subLedgers.get(id); return s && s.userId === userId ? s : undefined; }
  ...
  async getGroupActivity(id: number, userId: string) { const g = this.groupActivities.get(id); return g && g.userId === userId ? g : undefined; }
  ...
  async getLoan(id: number, userId: string) { const l = this.loans.get(id); return l && l.userId === userId ? l : undefined; }
  ...
  async initializeUserDefaults(userId: string, defaultCurrency = "MYR") {
    const existingWallets = await this.getWallets(userId, true);
