...
import { withPgLock, LOCK_REMINDERS } from "./pg-lock";
...
async function snapshotMonthlyBalances(): Promise<void> {
  // Run on the 1st of the month (any hour). The unique index + onConflictDoNothing make
  // this idempotent, so we no longer restrict to the 00:00 hour — that meant a server that
  // was down during that single hour would miss the whole month's snapshot.
  const now = new Date();
...
async function tick(): Promise<void> {
  // Only one instance runs these per tick (autoscale-safe) — avoids duplicate push
  // notifications and duplicate snapshot attempts.
  await withPgLock(LOCK_REMINDERS, async () => {
