...
import { storage, isMock } from "./storage";
  ...
  if (isLocalAuth) {
    ...
    app.post("/api/register", async (req, res) => {
      try {
        ...
        if (isMock) {
          ...
          const created = { id, email, passwordHash: hashPassword(password), firstName, lastName, profileImageUrl: null, defaultCurrency: "MYR", createdAt: new Date(), updatedAt: new Date() };
          await storage.upsertUser(created);
          return res.status(201).json(created);
        }
        const existing = await (db as any).select().from(users).where(eq(users.email, email));
    ...
    app.post("/api/login", async (req, res) => {
      try {
        ...
        let user;
        if (isMock) {
          // For mock, just allow login if any user matches the password locally, or create dummy
          user = { id: "demo-user", email, passwordHash: hashPassword(password), firstName: "Demo", lastName: "User", defaultCurrency: "MYR" };
        } else {
          const rows = await (db as any).select().from(users).where(eq(users.email, email));
          user = rows[0];
        }
