...
const IV_LENGTH = 16;          // legacy CBC IV length
const GCM_IV_LENGTH = 12;      // standard GCM nonce length
const ALGORITHM = 'aes-256-cbc';       // legacy (decrypt-only)
const GCM_ALGORITHM = 'aes-256-gcm';   // current (authenticated)
...
// Current format: "gcm:<ivHex>:<tagHex>:<cipherHex>" — AES-256-GCM (authenticated).
export function encrypt(text: string): string {
  const iv = crypto.randomBytes(GCM_IV_LENGTH);
  const cipher = crypto.createCipheriv(GCM_ALGORITHM, deriveKey(getEncryptionKey()), iv);
...
function decryptGcm(ivHex: string, tagHex: string, encrypted: string, key: string): string {
  const decipher = crypto.createDecipheriv(GCM_ALGORITHM, deriveKey(key), Buffer.from(ivHex, 'hex'));
...
function decryptCbc(ivHex: string, encrypted: string, key: string): string {
...
export function decrypt(encryptedText: string): string {
  const keys = [getEncryptionKey(), getLegacyKey()].filter(Boolean) as string[];
  ...
  if (encryptedText.startsWith('gcm:')) {
    ...
    let lastErr: unknown;
    for (const k of keys) {
      try { return decryptGcm(ivHex, tagHex, encrypted, k); } catch (e) { lastErr = e; }
    }
    throw lastErr || new Error('Decryption failed');
  }

  // Legacy CBC format: "<ivHex>:<cipherHex>" (kept for backward compatibility;
  // re-saving a credential upgrades it to GCM).
  const [ivHex, encrypted] = encryptedText.split(':');
  if (!ivHex || !encrypted) throw new Error('Invalid encrypted text format');
  let lastErr: unknown;
  for (const k of keys) {
    try { return decryptCbc(ivHex, encrypted, k); } catch (e) { lastErr = e; }
  }
  throw lastErr || new Error('Decryption failed');
}
