...
export interface IStorage {
  ...
  // AI insights cache
  getLatestAiInsights(userId: string): Promise<{ payload: any; createdAt: Date } | undefined>;
  saveAiInsights(userId: string, payload: any): Promise<void>;

  // Wallet operations
  getWallets(userId: string, includeArchived?: boolean): Promise<Wallet[]>;
  ...
  getTransactionsByTag(userId: string, tag: string): Promise<Transaction[]>;
  ...
  // Atomic money operations: create/update/delete a transaction AND adjust wallet
  // balances inside a single DB transaction so a mid-way failure can never leave the
  // ledger and the wallet balances out of sync.
  createTransactionWithEffects(transaction: InsertTransaction, deltas: WalletDelta[]): Promise<Transaction>;
  updateTransactionWithEffects(
    id: number,
    userId: string,
    data: Partial<InsertTransaction>,
    reversalDeltas: WalletDelta[],
    applyDeltas: WalletDelta[],
  ): Promise<Transaction | undefined>;
  deleteTransactionWithEffects(id: number, userId: string, reversalDeltas: WalletDelta[]): Promise<boolean>;
...
// A single wallet balance adjustment (positive = credit, negative = debit).
...
export class DatabaseStorage implements IStorage {
  ...
  // Wallet operations
  async getWallets(userId: string, includeArchived: boolean = false): Promise<Wallet[]> {
    if (includeArchived) {
      return db.select().from(wallets).where(eq(wallets.userId, userId));
    }
    // Exclude archived (soft-deleted) wallets from active lists and asset totals.
  ...
  async getTransactions(userId: string, filters?: TransactionFilters): Promise<any[]> {
    ...
    let query = db
      ...
      .from(transactions)
      // Join conditions include userId so a transaction that (defensively) references a
      // row owned by someone else can never surface another user's category/wallet names.
      .leftJoin(categories, and(eq(transactions.categoryId, categories.id), eq(categories.userId, userId)))
      .leftJoin(wallets, and(eq(transactions.walletId, wallets.id), eq(wallets.userId, userId)))
      .leftJoin(subLedgers, and(eq(transactions.subLedgerId, subLedgers.id), eq(subLedgers.userId, userId)))
      .leftJoin(toWallets, and(eq(transactions.toWalletId, toWallets.id), eq(toWallets.userId, userId)))
  ...
  // Apply a set of balance adjustments within an open transaction handle.
  private async applyDeltasTx(tx: any, userId: string, deltas: WalletDelta[]): Promise<void> {
    for (const d of deltas) {
      if (!d || !d.walletId || !Number.isFinite(d.delta) || d.delta === 0) continue;
      const deltaStr = d.delta.toFixed(2);
      await tx
        .update(wallets)
        .set({ balance: sql`${wallets.balance} + ${deltaStr}::numeric` })
        .where(and(eq(wallets.id, d.walletId), eq(wallets.userId, userId)));
    }
  }

  async createTransactionWithEffects(transaction: InsertTransaction, deltas: WalletDelta[]): Promise<Transaction> {
    return await db.transaction(async (tx: any) => {
      // Insert first so any FK violation aborts the whole tx before balances move
      // (prevents "balance debited but no transaction recorded").
      const [row] = await tx.insert(transactions).values(transaction).returning();
      await this.applyDeltasTx(tx, transaction.userId as string, deltas);
      return row;
    });
  }

  async updateTransactionWithEffects(
    id: number,
    userId: string,
    data: Partial<InsertTransaction>,
    reversalDeltas: WalletDelta[],
    ...
    return await db.transaction(async (tx: any) => {
      await this.applyDeltasTx(tx, userId, reversalDeltas);
      const [row] = await tx
        .update(transactions)
        .set(data)
        .where(and(eq(transactions.id, id), eq(transactions.userId, userId)))
        .returning();
      if (!row) {
        // Row vanished/not owned — abort so the reversal is rolled back.
        throw new Error("Transaction not found during atomic update");
      }
      await this.applyDeltasTx(tx, userId, applyDeltas);
      return row;
    });
  }

  async deleteTransactionWithEffects(id: number, userId: string, reversalDeltas: WalletDelta[]): Promise<boolean> {
    return await db.transaction(async (tx: any) => {
  ...
  async getBudgetSpending(userId: string, month: number, year: number): Promise<BudgetWithSpending[]> {
    ...
    // PERF: one scan of the month's expenses + one categories fetch, instead of a
    // per-budget transaction+category query (was 2N queries).
    const monthExpenses = await this.getTransactions(userId, { startDate, endDate, type: 'expense' });
    const spentByCategory = new Map<number, number>();
    for (const t of monthExpenses) {
      // budgets are denominated in the user's default currency, but transactions may live
      // in wallets of different currencies — convert via the wallet's exchange rate.
      if (t.loanId || !t.categoryId) continue;
      const rate = parseFloat(t.wallet?.exchangeRateToDefault || '1');
      const safeRate = isNaN(rate) || rate <= 0 ? 1 : rate;
      const amt = parseFloat(t.amount) * safeRate;
  ...
  async initializeUserDefaults(userId: string, defaultCurrency: string = "MYR"): Promise<void> {
    // Ensure default wallets exist (count archived too, so archiving every wallet
    // doesn't cause defaults to be silently recreated).
