...
// SECURITY: validate the DB server's TLS certificate by default. Only disable it
// explicitly (DB_SSL_INSECURE=true) for environments with a self-signed/untrusted chain.
const sslInsecure = process.env.DB_SSL_INSECURE === "true";
const sslConfig = { rejectUnauthorized: !sslInsecure };
if (sslInsecure) {
  console.warn("[db] DB_SSL_INSECURE=true — TLS certificate verification is DISABLED for the database connection.");
}
