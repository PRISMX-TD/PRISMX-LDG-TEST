...
export const wallets = pgTable("wallets", {
  ...
  // Soft-delete: when a wallet is "deleted" while keeping its transactions, we archive it
  // instead of hard-deleting (which would cascade-delete its transactions). Archived wallets
  // are hidden from active lists and asset totals but remain queryable for history.
...
export const savingsGoals = pgTable("savings_goals", {
  ...
  // FIX (audit #6): when set, the savings goal mirrors this wallet's balance
  // instead of being a free-floating number. Transfers into the wallet move the goal forward.
  linkedWalletId: integer("linked_wallet_id").references(() => wallets.id, { onDelete: "set null" }),
  createdAt: timestamp("created_at").defaultNow(),
});
...
export const subLedgers = pgTable("sub_ledgers", {
  ...
  isArchived: boolean("is_archived").default(false),
  currency: varchar("currency", { length: 10 }).notNull().default("MYR"), // preferred currency for display
...
// Password reset tokens (single-use, short-lived). Stored hashed to prevent
// leaking valid reset links if the table is ever exfiltrated.
export const passwordResetTokens = pgTable("password_reset_tokens", {
  ...
  index("idx_password_reset_token").on(t.tokenHash),
]);

export type PasswordResetToken = typeof passwordResetTokens.$inferSelect;
export type InsertPasswordResetToken = typeof passwordResetTokens.$inferInsert;

// Web push subscriptions. One user may register multiple endpoints (e.g., phone + laptop).
...
// Monthly per-user net asset snapshots. Populated by a cron tick on the 1st of every month
// so the Dashboard's month-over-month trend can show real deltas instead of the fake
// "current minus this-month income/expense" approximation.
export const monthlyBalanceSnapshots = pgTable("monthly_balance_snapshots", {
  id: integer("id").primaryKey().generatedAlwaysAsIdentity(),
  userId: varchar("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  year: integer("year").notNull(),
  ...
  uniqueIndex("uniq_balance_snapshot_user_period").on(t.userId, t.year, t.month),
]);
