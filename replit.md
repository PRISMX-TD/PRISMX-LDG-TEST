...
- **sessions**: 会话存储表
- **wallets**: 钱包表 (id, userId, name, type, currency, balance, exchangeRateToDefault, icon, color, isDefault, isFlexible, isArchived, archivedAt)
...
- `AUTH_SECRET` - 会话 token 的 HMAC 密钥，**必须固定且跨实例一致**（如 `openssl rand -hex 32`）。未设置时生产会启动失败；开发环境回退为每进程随机密钥。
- `PUBLIC_APP_URL` - 站点基础 URL（如 `https://your-app.com`），用于生成密码重置链接。**不设置则回退到请求 Host 头，存在重置链接投毒风险，生产务必设置。**
- `ENCRYPTION_KEY` - 交易所 API 凭证的加密密钥（≥16 位）。未设置时回退用 `SESSION_SECRET`。

### 可选
- `DB_SSL_INSECURE=true` - 关闭数据库 TLS 证书校验（默认开启校验）。仅当某环境证书链不受信任时使用。
- `DEEPSEEK_API_KEY` - AI 财务体检（未配则仅返回确定性指标）。
- `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` - Web Push 推送。
- `BREVO_API_KEY` / `SMTP_*` - 邮件发送（密码重置邮件）。
- `DISABLE_AUTH=true` - **仅本地开发**免登录调试（生产环境强制忽略）。
- `DISABLE_RECURRING_SCHEDULER` / `DISABLE_REMINDERS_SCHEDULER` - 关闭后台调度器。
...
## 开发命令
- `npm run dev` - 启动开发服务器
- `npm run db:push` - 推送数据库模式（新增了 `wallets.is_archived` 列，部署前需执行）
